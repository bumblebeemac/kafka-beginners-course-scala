...
import org.apache.kafka.clients.consumer.{ConsumerConfig, ConsumerRecords, CooperativeStickyAssignor, KafkaConsumer}
...
object ConsumerDemoCooperative {
  ...
  def demoKafkaConsumerWithShutdown(): Unit = {
    ...
    properties.setProperty(ConsumerConfig.PARTITION_ASSIGNMENT_STRATEGY_CONFIG,
      classOf[CooperativeStickyAssignor].getCanonicalName)
    ...
    try {
      ...
      case _: WakeupException => logger.info("Wake up exception")
